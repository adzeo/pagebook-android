...
public class BusinessPostsRecyclerViewAdapter extends RecyclerView.Adapter<BusinessPostsRecyclerViewAdapter.ViewHolder> {

    private final List<PostDTO> mPostsDataList;
    private final BusinessProfilePageActivity businessProfilePageActivity;

    User myUser = UserBuilder.getInstance();

    public BusinessPostsRecyclerViewAdapter(List<PostDTO> userDataList, BusinessProfilePageActivity businessProfilePageActivity) {
        this.mPostsDataList = userDataList;
        this.businessProfilePageActivity = businessProfilePageActivity;
    }

    @NonNull
    @Override
    public BusinessPostsRecyclerViewAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.post_item, parent, false);
        return new BusinessPostsRecyclerViewAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull BusinessPostsRecyclerViewAdapter.ViewHolder holder, int position) {
                ...
                responses.enqueue(new Callback<Post>() {
                    ...
                    public void onResponse(Call<Post> call, retrofit2.Response<Post> responseData) {
                        if (responseData.body() != null) {
                            Toast.makeText(businessProfilePageActivity, "Post Shared", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(businessProfilePageActivity, "Error", Toast.LENGTH_SHORT).show();
                        }
                    }

                    @Override
                    public void onFailure(Call<Post> call, Throwable t) {
                        Toast.makeText(businessProfilePageActivity, t.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
            }
        });
