...
public class FriendsProfilePostsRecyclerViewAdapter extends RecyclerView.Adapter<FriendsProfilePostsRecyclerViewAdapter.ViewHolder> {

    private final List<PostDTO> mPostsDataList;
    private final FriendsProfileActivity friendsProfileActivity;

    User myUser = UserBuilder.getInstance();

    public FriendsProfilePostsRecyclerViewAdapter(List<PostDTO> userDataList, FriendsProfileActivity friendsProfileActivity) {
        this.mPostsDataList = userDataList;
        this.friendsProfileActivity = friendsProfileActivity;
    }

    @NonNull
    @Override
    public FriendsProfilePostsRecyclerViewAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.post_item, parent, false);
        return new FriendsProfilePostsRecyclerViewAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull FriendsProfilePostsRecyclerViewAdapter.ViewHolder holder, int position) {
        ...
        holder.llComments.setOnClickListener(v -> {
            Intent intent = new Intent(friendsProfileActivity, CommentsActivity.class);
                ...
                responses.enqueue(new Callback<Post>() {
                    ...
                    public void onResponse(Call<Post> call, retrofit2.Response<Post> responseData) {
                        if (responseData.body() != null) {
                            Toast.makeText(friendsProfileActivity, "Post Shared", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(friendsProfileActivity, "Error", Toast.LENGTH_SHORT).show();
                        }
                    }

                    @Override
                    public void onFailure(Call<Post> call, Throwable t) {
                        Toast.makeText(friendsProfileActivity, t.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
            }
        });
